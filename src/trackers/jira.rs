    ...
    async fn create_followup(&self, parent_id: &str, content: FollowUpContent) -> Result<String> {
        ...
        let body = json!({
            "fields": {
                ...
                "description": text_to_adf(&content.description),
                "issuetype": { "name": "Task" },
                "labels": content.labels,
            }
        });
        let created = self.post_json("/rest/api/3/issue", body).await?;
        let new_key = created["key"]
            .as_str()
            .ok_or_else(|| {
                HiveError::Tracker("Jira create issue did not return a key".into())
            })?
            .to_string();

        // Best-effort link to the parent. A failed link should not roll back
        // the created issue — the caller still gets a valid key back.
        let link_body = json!({
            "type": { "name": "Relates" },
            "inwardIssue": { "key": new_key },
            "outwardIssue": { "key": parent_id },
        });
        if let Err(e) = self.post_json("/rest/api/3/issueLink", link_body).await {
            tracing::warn!(parent = parent_id, child = %new_key, err = %e, "Jira issueLink failed");
        }
...
/// separate paragraphs. Sufficient for hive's follow-up issues, which are
/// short structured text generated by the runner.
...
mod tests {
    ...
    fn text_to_adf_roundtrip() {
        ...
        let flat = adf_to_text(&adf);
        assert!(flat.contains("Hello world"));
        assert!(flat.contains("Second para"));
    }
}
